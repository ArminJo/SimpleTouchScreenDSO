 ...
 *
 *      Build on:
 *      Arduino Uno Rev 3 http://www.watterott.com/de/Arduino-Uno           25 EUR
 *      mSD-Shield http://www.watterott.com/de/Arduino-mSD-Shield           16 EUR
 *      MI0283QT-2 Adapter http://www.watterott.com/de/MI0283QT-2-Adapter   36 EUR
 *      Total                                                               77 EUR
 *
 ...
 *
 * INFO LINE
 * A A 5 3     - Slope of trigger A=ascending D=descending
 * 			   - Mode of trigger A=automatic M=manual (manual currently means last automatic values are taken and not changed anymore)
 * 			   - Reference used 5=5V 1  1=1.1Volt-internal-reference
 * 			   - Number of input channel (1-5) and T=AVR-temperature R=1.1Volt-internal-reference G=internal-ground
 * 1.35 2.40 4.38 3.03V - Min, arithmetic-average, max and peak to peak voltage of actual chart (In hold mode, chart is larger than display!)
 * 101us       - Time for div (31 pixel)
 * 0.86	- Trigger auto => Value of actual trigger level (not updated)
 *
 * SECOND LINE
 * 20ms	 - Period
 * 50Hz	 - Frequency
...
/*
 * since 5.0 / 1024.0 = 0,00488 Volt is the resolution of the ADC
 * depending of scale factor
 * the printed values for 2 adjacent display (min/max) values can be the same
...
inline void toggleTimingDebug(void) {
    TIMING_DEBUG_PORT_INPUT = (1 << TIMING_DEBUG_PIN);
}
inline void setTimingDebug(void) {
    TIMING_DEBUG_PORT |= (1 << TIMING_DEBUG_PIN);
}
inline void resetTimingDebug(void) {
    TIMING_DEBUG_PORT &= ~(1 << TIMING_DEBUG_PIN);
}
#endif
...
bool TouchPanelGetPressureValid = false; // Flag to indicate that TouchPanel.service was just called before
// Control variables
boolean StartNewTouchLocal = true; // only one button handling in loop each touching of local display
...
const char ReferenceButton5_0V[] PROGMEM = "Ref 5.0V";
...
#endif
char ChannelSelectButtonString[] = "Channel 3";
...
const char ChartHistoryButtonStringOff[] PROGMEM = "Hist off";
const char ChartHistoryButtonStringLow[] PROGMEM = "Hist low";
const char ChartHistoryButtonStringOn[] PROGMEM = "Hist on";
 ...
 */
#define DISPLAY_MODE_SHOW_GUI 0
#define DISPLAY_MODE_CHART 1
 ...
 */
char StringBuffer[40] __attribute__((section(".noinit")));
 ...
 * Data buffer
 */
#define DATABUFFER_DISPLAY_INCREMENT (LOCAL_DISPLAY_WIDTH/4)
...
void drawGrid(void);
void drawDataBuffer(uint8_t *DataBufferPointer, uint16_t aColor, uint16_t aClearBeforeColor);
...
void initDisplay(void) {
    ...
    BlueDisplay1.setCharacterMapping(0xF5, 0x00B5); // mapping for micro - use F5 instead of B5 since 75 is an 'u'
}
...
//  LocalDisplay.init(4); //spi-clk = Fcpu/4
    ...
    if (TouchPanel.getPressure() > 5) {
        TouchPanel.doCalibration(&LocalDisplay, TP_EEPROMADDR, 0); //dont check EEPROM for calibration data
    } else {
        TouchPanel.doCalibration(&LocalDisplay, TP_EEPROMADDR, 1); //check EEPROM for calibration data
    }
    ...
    for (;;) {
        ...
        if (MeasurementControl.IsRunning) {
            ...
            if (DataBufferControl.DataBufferFull) {
                ...
                TouchPanel.service();
                TouchPanelGetPressureValid = true;
...
//            printDebugData();
            ...
            if (MeasurementControl.isSingleShotMode && MeasurementControl.searchForTrigger
                ...
                TouchPanel.service(); // 22 / 270  Microseconds without/with touch
                TouchPanelGetPressureValid = true;
            }
             ...
             */
            if (!TouchPanelGetPressureValid && MeasurementControl.TimebaseJustDelayed
        ...
        if (tPressure < MIN_PRESSURE) {
            /**
             * no touch here
             */
            StartNewTouchLocal = true;
            ButtonPressedFirst = false;
        }
        if (TouchPanelGetPressureValid) {
            TouchPanelGetPressureValid = false;
            if (tPressure >= MIN_PRESSURE) {
                uint16_t tXPos = TouchPanel.getX();
                uint16_t tYPos = TouchPanel.getY();
                /*
                ...
                if (StartNewTouchLocal) {
                    tGuiTouched = TouchButton::checkAllButtons(tXPos, tYPos);
                    if (tGuiTouched) {
                ...
                if (!tGuiTouched && !ButtonPressedFirst) {
                    ...
                    if (tGuiTouched && StartNewTouchLocal) {
                        ButtonPressedFirst = false;
                    }
                }
                if (tGuiTouched) {
                    StartNewTouchLocal = false;
                }
...
void redrawDisplay() {
    ...
    if (MeasurementControl.IsRunning) {
        ...
        // analyze mode
        if (DisplayControl.DisplayMode == DISPLAY_MODE_SHOW_GUI) {
            drawGui();
...
bool changeInputRange(uint8_t aValue) {
    ...
    // change grid if leaving or entering highest resolution with grid size 41
...
void createGUI(void) {
    ...
    TouchButtonBack_Singleshot.initButton(BUTTON_WIDTH_3_POS_3, 0, BUTTON_WIDTH_3, BUTTON_HEIGHT_4, COLOR_GUI_CONTROL,
            ButtonStringSingleshot,
            TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0, &doTriggerSingleshot);

// big start stop button
    TouchButtonStartStop.initButton(BUTTON_WIDTH_3_POS_3, BUTTON_HEIGHT_4_LINE_2, BUTTON_WIDTH_3,
            2 * BUTTON_HEIGHT_4 + BUTTON_SPACING, COLOR_GUI_CONTROL, PSTR("start/stop"), TEXT_SIZE_11,
            FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0, &doStartStop);

    // Button for settings page
    TouchButtonSettings.initButton(BUTTON_WIDTH_3_POS_3, BUTTON_HEIGHT_4_LINE_4, BUTTON_WIDTH_3, BUTTON_HEIGHT_4, COLOR_GUI_CONTROL,
            PSTR("Settings"), TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0, &doSettings);

    // Buttons for timebase +/-
    TouchButtonRight.initButton(BUTTON_WIDTH_5_POS_2, BUTTON_HEIGHT_4_LINE_4, BUTTON_WIDTH_5, BUTTON_HEIGHT_4,
    COLOR_GUI_SOURCE_TIMEBASE, ">", TEXT_SIZE_22, FLAG_BUTTON_DO_BEEP_ON_TOUCH, 1, &doTimeBaseDisplayScroll);

    TouchButtonLeft.initButton(0, BUTTON_HEIGHT_4_LINE_4, BUTTON_WIDTH_5, BUTTON_HEIGHT_4, COLOR_GUI_SOURCE_TIMEBASE, "<",
...
// Button for switching display mode
    TouchButtonDisplayMode.initButton(0, 0, BUTTON_WIDTH_3, BUTTON_HEIGHT_4, COLOR_GUI_DISPLAY_CONTROL, PSTR("Display"),
    TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0, &doDisplayMode);

// Button for chart history (erase color)
    TouchButtonChartHistory.initButton(BUTTON_WIDTH_5_POS_3, BUTTON_HEIGHT_4_LINE_4,
            LOCAL_DISPLAY_WIDTH - (BUTTON_WIDTH_3_POS_2) - (BUTTON_WIDTH_5_POS_3), BUTTON_HEIGHT_4, COLOR_GUI_DISPLAY_CONTROL,
            ChartHistoryButtonStrings[0], TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0,
            &doChartHistory);
    ...
    TouchButtonAutoTriggerOnOff.initButton(BUTTON_WIDTH_3_POS_2, 0, BUTTON_WIDTH_3, BUTTON_HEIGHT_4, COLOR_GUI_TRIGGER,
            AutoTriggerButtonStringAuto, TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0,
            &doTriggerAutoOnOff);

    // Button for slope
    TouchButtonSlope.initButton(BUTTON_WIDTH_3_POS_2, BUTTON_HEIGHT_4_LINE_2, BUTTON_WIDTH_3, BUTTON_HEIGHT_4,
    COLOR_GUI_TRIGGER, SlopeButtonString, TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH, 0, &doTriggerSlope);

    // Button for range
    TouchButtonAutoRangeOnOff.initButton(BUTTON_WIDTH_3_POS_2, BUTTON_HEIGHT_4_LINE_3, BUTTON_WIDTH_3,
    BUTTON_HEIGHT_4, COLOR_GUI_TRIGGER, AutoRangeButtonStringAuto, TEXT_SIZE_11,
            FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0, &doRangeMode);

    // Button for auto offset on off
    TouchButtonAutoOffsetOnOff.initButton(BUTTON_WIDTH_3_POS_2, BUTTON_HEIGHT_4_LINE_4, BUTTON_WIDTH_3, BUTTON_HEIGHT_4,
    COLOR_GUI_TRIGGER, AutoOffsetButtonString0, TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE, 0,
            &doAutoOffsetOnOff);
...
// Button for channel select
    TouchButtonChannelSelect.initButton(BUTTON_WIDTH_3_POS_3, BUTTON_HEIGHT_4_LINE_2, BUTTON_WIDTH_3, BUTTON_HEIGHT_4,
...
// Button for reference voltage switching
    TouchButtonADCReference.initButton(BUTTON_WIDTH_3_POS_3, BUTTON_HEIGHT_4_LINE_4, BUTTON_WIDTH_3, BUTTON_HEIGHT_4,
    COLOR_GUI_SOURCE_TIMEBASE, ReferenceButton5_0V, TEXT_SIZE_11, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE,
            0, &doADCReference);
     ...
     */
#ifdef LOCAL_DISPLAY_EXISTS
    TouchSliderBacklight.initSlider(40, 60, 16, BACKLIGHT_MAX_VALUE, BACKLIGHT_MAX_VALUE,
    BACKLIGHT_START_VALUE, "Backlight", SLIDER_DEFAULT_TOUCH_BORDER, TOUCHFLAG_SLIDER_SHOW_BORDER | TOUCHSLIDER_SHOW_VALUE,
            &doBacklight, NULL);
#endif
// make slider slightly visible
    ...
    TouchSliderVoltagePicker.initSlider(SLIDER_VPICKER_VALUE_X - TEXT_SIZE_11_WIDTH + 4, 0, 24, DISPLAY_VALUE_FOR_ZERO,
            LOCAL_DISPLAY_HEIGHT - 1, 0, NULL, SLIDER_DEFAULT_TOUCH_BORDER, TOUCHSLIDER_VERTICAL_SHOW_NOTHING, &doVoltagePicker,
            NULL);

    // invisible slider for trigger level
    TouchSliderTriggerLevel.initSlider(SLIDER_TLEVEL_VALUE_X - 4, 0, 24, DISPLAY_VALUE_FOR_ZERO, LOCAL_DISPLAY_HEIGHT - 1, 0, NULL,
    SLIDER_DEFAULT_TOUCH_BORDER, TOUCHSLIDER_VERTICAL_SHOW_NOTHING, &doTriggerLevel, NULL);
}

void drawGui(void) {
    DisplayControl.DisplayMode = DISPLAY_MODE_SHOW_GUI;
    BlueDisplay1.clearDisplay(COLOR_BACKGROUND_DSO);
...
void clearDiplayedChart(uint8_t * aDisplayBufferPtr) {
...
void doTimeBaseDisplayScroll(TouchButton * const aTheTouchedButton, int16_t aValue) {
    ...
    if (MeasurementControl.IsRunning) {
        ...
        if (tNewIndex < 0) {
            ...
            if (tOldIndex >= TIMEBASE_INDEX_DRAW_WHILE_ACQUIRE && tNewIndex < TIMEBASE_INDEX_DRAW_WHILE_ACQUIRE) {
                ...
                clearDiplayedChart(&DataBufferControl.DisplayBuffer[0]);
            ...
            if (tOldIndex < TIMEBASE_INDEX_DRAW_WHILE_ACQUIRE && tNewIndex >= TIMEBASE_INDEX_DRAW_WHILE_ACQUIRE) {
                ...
                ADCSRA &= ~(1 << ADIE); // stop acquisition - disable ADC interrupt
                clearDiplayedChart(&DataBufferControl.DataBuffer[0]);
                tStartNewAcquisition = true;
            }
    ...
    } else if (DisplayControl.DisplayMode != DISPLAY_MODE_SHOW_GUI) {
        /*
...
void doTriggerSlope(TouchButton * const aTheTouchedButton, int16_t aValue) {
    ...
    TouchButtonSlope.setCaption(SlopeButtonString);
    TouchButtonSlope.drawCaption();
}
...
// set channel number in caption
    ...
    TouchButtonChannelSelect.drawCaption();
}
...
void doDisplayMode(TouchButton * const aTheTouchedButton, int16_t aValue) {
    ...
    if (MeasurementControl.IsRunning) {
        ...
        if (DisplayControl.showInfoMode) {
            ...
            BlueDisplay1.fillRectRel(INFO_LEFT_MARGIN, 1, LOCAL_DISPLAY_WIDTH - INFO_LEFT_MARGIN,
                    (2 * TEXT_SIZE_11) + TEXT_SIZE_11_DECEND, COLOR_BACKGROUND_DSO);
            // draw grid
            drawGridLinesWithHorizLabelsAndTriggerLine();
        }
    } else {
...
void doTriggerSingleshot(TouchButton * const aTheTouchedButton, int16_t aValue) {
    ...
    if (DisplayControl.DisplayMode == DISPLAY_MODE_SETTINGS) {
        ...
        BlueDisplay1.drawChar(INFO_LEFT_MARGIN + SINGLESHOT_PPRINT_VALUE_X, INFO_UPPER_MARGIN + TEXT_SIZE_11_HEIGHT, 'S',
...
#ifdef LOCAL_DISPLAY_EXISTS
void doBacklight(TouchSlider * const aTheTouchedSlider, uint16_t aBrightness) {
    if (aBrightness < BACKLIGHT_MIN_VALUE) {
        aBrightness = BACKLIGHT_MIN_VALUE;
    }
    LocalDisplay.led(aBrightness);
}
#endif
 ...
 */
void doTriggerLevel(TouchSlider * const aTheTouchedSlider, uint16_t aValue) {
 ...
 * The value printed has a resolution of 0,00488 * scale factor
 */
void doVoltagePicker(TouchSlider * aTheTouchedSlider, uint16_t aValue) {
    ...
    if (LastPickerValue != 0xFF) {
        ...
        BlueDisplay1.drawLineRel(0, tYpos, LOCAL_DISPLAY_WIDTH, 0, COLOR_BACKGROUND_DSO);
...
// draw new line
    ...
    BlueDisplay1.drawLineRel(0, tValue, LOCAL_DISPLAY_WIDTH, 0, COLOR_DATA_PICKER);
...
void clearTriggerLine(uint8_t aTriggerLevelDisplayValue) {
    BlueDisplay1.drawLineRel(0, aTriggerLevelDisplayValue, LOCAL_DISPLAY_WIDTH, 0, COLOR_BACKGROUND_DSO);
    if (DisplayControl.showInfoMode) {
        // restore grid at old y position
        for (uint16_t tXPos = TIMING_GRID_WIDTH - 1; tXPos < LOCAL_DISPLAY_WIDTH - 1; tXPos += TIMING_GRID_WIDTH) {
...
void drawTriggerLine(void) {
    ...
    BlueDisplay1.drawLineRel(0, tValue, LOCAL_DISPLAY_WIDTH, 0, COLOR_TRIGGER_LINE);
}
...
// clear line
        BlueDisplay1.drawLineRel(0, tYPos, LOCAL_DISPLAY_WIDTH, 0, COLOR_BACKGROUND_DSO);
// clear label
        BlueDisplay1.fillRectRel(HORIZONTAL_LINE_LABELS_CAPION_X, tYPos - tCaptionOffset, (4 * TEXT_SIZE_11_WIDTH) - 1,
...
// vertical (timing) lines
    for (uint16_t tXPos = TIMING_GRID_WIDTH - 1; tXPos < LOCAL_DISPLAY_WIDTH; tXPos += TIMING_GRID_WIDTH) {
        BlueDisplay1.drawLineRel(tXPos, 0, 0, LOCAL_DISPLAY_HEIGHT, COLOR_TIMING_LINES);
    }
...
// draw first caption over the line
    ...
    for (int32_t tYPosLoop = DISPLAY_VALUE_FOR_ZERO_SHIFT8; tYPosLoop > 0; tYPosLoop -=
        ...
        uint16_t tYPos = tYPosLoop / 0x100;
// horizontal line
        BlueDisplay1.drawLineRel(0, tYPos, LOCAL_DISPLAY_WIDTH, 0, COLOR_TIMING_LINES);
        dtostrf(tActualVoltage, 4, 2, tStringBuffer);
// draw label over the line
        BlueDisplay1.drawText(HORIZONTAL_LINE_LABELS_CAPION_X, tYPos - tCaptionOffset, tStringBuffer, 11, COLOR_HOR_REF_LINE_LABEL,
        COLOR_NO_BACKGROUND);
...
// draw max line
    ...
    if (tValueDisplay != 0) {
        BlueDisplay1.drawLineRel(0, tValueDisplay, LOCAL_DISPLAY_WIDTH, 0, COLOR_MAX_MIN_LINE);
    }
// min line
...
/*
 * Draws on screen - if color is DATA_ERASE_COLOR then chart is removed :-)
 * Data is taken from DataBufferPointer
 * - 9ms for drawPixelfast
 */
void drawDataBuffer(uint8_t *DataBufferPointer, uint16_t aColor, uint16_t aClearBeforeColor) {
    uint16_t i;
    uint8_t *ScreenBufferWritePointer = &DataBufferControl.DisplayBuffer[0];
    uint8_t *ScreenBufferReadPointer = &DataBufferControl.DisplayBuffer[0];
    ...
    if (aClearBeforeColor > 0) {
        ...
        tLastValueClear = *ScreenBufferReadPointer++;
        tValueClear = *ScreenBufferReadPointer++;
        BlueDisplay1.drawLineFastOneX(0, tLastValueClear, tValueClear, aClearBeforeColor);
        tLastValueClear = tValueClear;
    }

    uint8_t tLastValue;
    tLastValue = *DataBufferPointer++;
    *ScreenBufferWritePointer++ = tLastValue;
    for (i = 0; i < LOCAL_DISPLAY_WIDTH - 1; ++i) {
        if (aClearBeforeColor > 0 && i != LOCAL_DISPLAY_WIDTH - 1) {
            ...
            tValueClear = *ScreenBufferReadPointer++;
            BlueDisplay1.drawLineFastOneX(i + 1, tLastValueClear, tValueClear, aClearBeforeColor);
            tLastValueClear = tValueClear;
        }
        // draw line
        tValueByte = *DataBufferPointer++;
        *ScreenBufferWritePointer++ = tValueByte;
        BlueDisplay1.drawLineFastOneX(i, tLastValue, tValueByte, aColor);
        tLastValue = tValueByte;
    }

    if (USART_isBluetoothPaired()) {
        BlueDisplay1.drawChartByteBuffer(0, 0, aColor, aClearBeforeColor, &DataBufferControl.DisplayBuffer[0],
                sizeof(DataBufferControl.DisplayBuffer));
    }
}
...
// 2 kByte code size
    ...
    sprintf_P(StringBuffer, PSTR("%3u%cs %c C%c %s %s %s %sV %sV %c"), tTimebaseUnitsPerGrid, tTimebaseUnitChar, tSlopeChar,
    ...
    if (tMicrosPeriod >= 1000) {
        ...
        StringBuffer[0] = StringBuffer[1];
        StringBuffer[1] = StringBuffer[2];
        StringBuffer[2] = THOUSANDS_SEPARATOR;
    }
    if (tHertz >= 1000) {
        // set separator for thousands
        StringBuffer[9] = StringBuffer[10];
        StringBuffer[10] = StringBuffer[11];
        StringBuffer[11] = THOUSANDS_SEPARATOR;
    }
//	 44 bytes
//	char * StringPointer = &StringBuffer[2];
//	char tChar;
...
void printTPData(bool aGuiTouched) {
    sprintf_P(StringBuffer, PSTR("X:%03i|%04i Y:%03i|%04i P:%03i"), TouchPanel.getX(), TouchPanel.getXraw(),
            TouchPanel.getY(), TouchPanel.getYraw(), TouchPanel.getPressure());
    uint16_t tColor = COLOR_BACKGROUND_DSO;
    if (aGuiTouched) {
        tColor = COLOR_GREEN;
    }
    drawText(INFO_LEFT_MARGIN, INFO_UPPER_MARGIN + 3 * TEXT_SIZE_11_HEIGHT, StringBuffer, 1, COLOR_BLACK, tColor);
}

void printDebugData(void) {
    sprintf_P(StringBuffer, PSTR("%5u,%5u,%5u,%5u"), DebugValue1, DebugValue2, DebugValue3, DebugValue4);
    drawText(INFO_LEFT_MARGIN, INFO_UPPER_MARGIN + 2 * TEXT_SIZE_11_HEIGHT, StringBuffer, 1, COLOR_BLACK, COLOR_BACKGROUND_DSO);
}
#endif
...
//buf[3] is day of week
    if (RtcBuf[0] != sLastSecond) {
        ...
        sprintf_P(StringBuffer, PSTR("%02i.%02i.%04i %02i:%02i:%02i"), RtcBuf[4], RtcBuf[5], RtcBuf[6] + 2000, RtcBuf[2], RtcBuf[1],
                RtcBuf[0]);
        BlueDisplay1.drawText(INFO_LEFT_MARGIN, INFO_UPPER_MARGIN + 2 * 13, StringBuffer, 11, COLOR_RED, COLOR_BACKGROUND_DSO);
    }
}
#endif

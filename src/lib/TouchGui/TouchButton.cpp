...
#include "MI0283QT2.h" // for LOCAL_DISPLAY
    ...
    if (mCaptionSize > 0) { // don't render anything if caption size == 0
        if (mCaption != NULL) {
            ...
            if (mFlags & FLAG_BUTTON_CAPTION_IS_IN_PGMSPACE) {
                LocalDisplay.drawTextPGM(tXCaptionPosition, tYCaptionPosition, (char *) mCaption, getLocalTextSize(mCaptionSize),
                        mCaptionColor, mButtonColor);
            } else {
                LocalDisplay.drawText(tXCaptionPosition, tYCaptionPosition, (char *) mCaption, getLocalTextSize(mCaptionSize),
                        mCaptionColor, mButtonColor);
            }
        }
    }
...
bool TouchButton::checkButton(uint16_t aTouchPositionX, uint16_t aTouchPositionY) {
    if ((mFlags & FLAG_IS_ACTIVE) && mOnTouchHandler != NULL && checkButtonInArea(aTouchPositionX, aTouchPositionY)) {
         ...
         */
        if (mFlags & FLAG_DO_BEEP_ON_TOUCH) {
            FeedbackToneOK();
        }

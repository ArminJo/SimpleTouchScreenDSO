...
#define FUNCTION_TAG_FILL_RECT          0x12
...
MI0283QT2::MI0283QT2(void) { // @suppress("Class members should be properly initialized")
    return;
}
...
# endif
    ...
    reset();
    p_size = 0;
    p_fg = 0X0000; //COLOR_BLACK;
    p_bg = 0XFFFF; //COLOR_WHITE;
...
void MI0283QT2::clearDisplay(uint16_t color) {
    uint16_t size;
...
void MI0283QT2::fillRectRel(uint16_t aXStart, uint16_t aYStart, uint16_t aWidth, uint16_t aHeight, uint16_t aColor) {
...
#ifdef FONT_END7F
    i = (uint8_t) (c & 0x7F);  // mask highest bit
#else
            i = (uint8_t) c;
#endif
